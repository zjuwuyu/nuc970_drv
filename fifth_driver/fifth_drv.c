...
int fifth_drv_init(void)
{
  ...
  if (major != 0)
    register_chrdev(major, "fifth_drv", &fifth_drv_ops);
  else
    major = register_chrdev(0, "fifth_drv", &fifth_drv_ops);
  ...
  for (i = 0; i < KEY_NUM; i++)
  {
    key_dev[i] = device_create(fifthdrv_class, NULL, MKDEV(major, i), NULL, "button%d", i);
    if (IS_ERR(key_dev[i]))
      goto error;
  }
...
{ 
  ...
  for (i = 0; i < KEY_NUM; i++)
  {
    device_destroy(fifthdrv_class, MKDEV(major, i));
  }
