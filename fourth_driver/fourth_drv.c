...
static struct file_operations fourth_drv_ops = {
	.owner = THIS_MODULE,
...
int fourth_drv_init(void)
{
  ...
  if (major != 0)
    register_chrdev(major, "fourth_drv", &fourth_drv_ops);
  else
    major = register_chrdev(0, "fourth_drv", &fourth_drv_ops);

  fourthdrv_class = class_create(THIS_MODULE, "fourth_drv");
  if (IS_ERR(fourthdrv_class))
    goto error;

  for (i = 0; i < KEY_NUM; i++)
  {
    key_dev[i] = device_create(fourthdrv_class, NULL, MKDEV(major, i), NULL, "button%d", i);
    if (IS_ERR(key_dev[i]))
      goto error;
  }
...
{ 
  ...
  for (i = 0; i < KEY_NUM; i++)
  {
    device_destroy(fourthdrv_class, MKDEV(major, i));
  }
